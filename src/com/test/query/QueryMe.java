    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            case VALUE_LIKE: 
                ...
                break;
            case VALUE_STRING: 
                // 3 value + string模块，大小写敏感，无需全词匹配，可根据部分字符串查询。搜索出所有包含指定STR字符串（也就是NAME字符串）的Topic
            ...
            case VALUE_LIKE_STRING:// ERROR！  
                // 4 value-like + string模块，value-like 的第二个变量必须被绑定，故不能使用string module
            ...
            // sss = "topic($topic)?";
            // sss = "select $name from topic-name($topic, $name)," +"value($name, \"王春\")?";
            // sss = "select $name from topic-name($topic, $name), value-like($name, \"王春\")?";
            // sss = "select $s from occurrence($topic, $OCC), value($OCC, \"川西致密气藏水平井控水压裂技术研究及应用.pdf\"), topic-name($topic, $name), value($name, $s)?";
            // sss = "import \"http://psi.ontopia.net/tolog/string/\" as str  topic-name($TOPIC, $n ), value($n ,$SI), str:contains($SI, \"王春\")?\"";
            // sss = "import \"http://psi.ontopia.net/tolog/string/\" as str select $s from occurrence($topic, $OCC), value($OCC,$ss), str:contains($ss, \"川西致密气藏水平井控水压裂技术研究及应用\"), topic-name($topic, $name), value($name, $s)?\"";
            sss = ss;
